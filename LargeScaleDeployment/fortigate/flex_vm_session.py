...
class FlexVMSession:
  ...
  def update_bearer_token(self):
      ...
      if response.ok and response.json()['status'] == 'success':
        ...
        print ("Error: Not possible to obtain bearer token")
        sys.exit(-1)
  ...
  #
  def create_vm_license(self, configId, count, description, endDate):
  ...
  #
  def update_vm_license(self, serialNumber, configId, description, endDate):
  ...
  #   endDate
  #
  def get_vms_licenses(self, configId):
  ...
  def regenerate_token(self, serialNumber):
    ...
    response = requests.request("POST", url, headers=headers, data=payload)
    return response.json()['vms'][0]['token']
...
if __name__ == '__main__':
  ...
  fvs = FlexVMSession("user", "passwd")
  fvs.update_bearer_token()

  vms = fvs.get_vms_licenses(257)
  vms.sort(key=fvs.getSN)

  for vm in vms:
    print(f" key: {vm['serialNumber']} token: {vm['token']} description: {vm['description']} Status: {vm['tokenStatus']}")
