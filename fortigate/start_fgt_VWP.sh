...
export CLIENT_IP=192.168.70.43
export CLIENT_MAC=08:00:27:4c:70:43

export SERVER_IP=192.168.80.44
export SERVER_MAC=08:00:27:4c:80:44
...
while [ $retries -gt 0 ]
do
    result=$(ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null user@${CLIENT_IP}  'sudo ip route | grep 192.168.80')
    if [ $? -eq 0 ] ; then
        break
    fi
    echo "Installing route in client..."
    ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null user@${CLIENT_IP}  sudo ip route add 192.168.80.0/24 dev ens4
    ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null user@${CLIENT_IP}  sudo arp -s 192.168.80.44 08:00:27:4c:80:44
    sleep 5
    retries=$((retries-1))
done
...
while [ $retries -gt 0 ]
do
    result=$(ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null user@${SERVER_IP}  'sudo apt -y install python;
    sudo ip route add 192.168.70.0/24 dev ens4;
    sudo arp -s 192.168.70.43 08:00:27:4c:70:43')
    if [ $? -eq 0 ] ; then
        break
    fi
    echo "Installing python in client..."
    sleep 5
    retries=$((retries-1))
done

ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null user@${SERVER_IP}  sudo python -m SimpleHTTPServer 80 &
